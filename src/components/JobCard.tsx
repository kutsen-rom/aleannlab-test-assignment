...
export default function JobCard({ job }: IProps) {
  ...
  and then iterate through it to render needed amount of stars*/}
            ...
            {/* JOB TITLE */}
            <h2 className="job-title text-brand text-lg h-12 w-[90%] overflow-hidden leading-6 sm:text-xl sm:h-14 md:font-semibold md:h-16 lg:h-12 lg:text-lg  lg:font-bold lg:mt-[1.65rem] lg:tracking-[-0.625px] lg:text-[#616A81] lg:leading-[1.375rem] lg:w-[100%] lg:mr-0 xl:text-xl xl:leading-[1.375rem]">
              {job.title}
            </h2>

            {/* DEPARTMENT NAME */}
            <h3 className="department-name text-muted tracking-[0.23619px] mt-[0.375rem] lg:text-[#9FA4B1] lg:font-light ">
              Department name •&nbsp;
            ...
            <div className="container-address flex items-center my-2 pb-[1.1875rem] lg:pb-[1rem]">
              ...
              {/* IMAGE: GEO MARKER */}
              <div className="h-[1.125rem] w-[1.125rem] bg-no-repeat bg-[url('../public/images/geo.svg')] lg:bg-[url('../public/images/geo-lighter.svg')]"></div>
  ...
  and then iterate through it to render needed amount of stars*/}
            {Array(rating)
              ...
              .map((el, index) => (
                ...
                <img
                  key={index}
                  alt="star"
                  className="lg-image-star mx-[0.03125rem] w-[0.625rem] h-[0.625rem] lg:w-4 lg:h-4 xl:w-5 xl:h-5  lg:mx-0"
          ...
          <div className="lg-container-save-button-and-posted w-full h-[90%] flex flex-col justify-between mr-1">
            ...
            <Link
              to=""
              className="lg-button-save self-end mr-[0.375rem] px-3 py-3 rounded-md transition duration-75 hover:bg-background active:bg-white focus:outline-brand focus:outline-offset-2"
